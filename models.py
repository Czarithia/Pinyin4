...
def Conv2D(N_CLASSES=56, SR=16000, DT=1.6):
    ...
    i = get_melspectrogram_layer(input_shape=input_shape,
                                 ...
                                 output_data_format='channels_last')(input_tensor) 
...
def Conv1D(N_CLASSES=56, SR=16000, DT=1.6):
    input_shape = (int(SR * DT), 1)
    input_tensor = layers.Input(shape=input_shape, name='input')
    i = get_melspectrogram_layer(input_shape=input_shape,
                                 n_mels=128,
    ...
    model.compile(optimizer='adam',
                  ...
                  metrics=['accuracy'])
    return model
def LSTM(N_CLASSES=56, SR=16000, DT=1.6):
    input_shape = (int(SR * DT), 1)
    input_tensor = layers.Input(shape=input_shape, name='input') 
    i = get_melspectrogram_layer(input_shape=input_shape,
                                 n_mels=128,
